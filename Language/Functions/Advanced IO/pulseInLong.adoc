...
Lê um pulso (ALTO ou BAIXO) em um pino. Por exemplo, se o valor for HIGH, `pulseInLong ()` espera que o pino vá `HIGH`, começa a cronometrar e, em seguida, espera que o pino vá` LOW` e pára de cronometrar. Retorna a duração do pulso em microssegundos ou 0 se nenhum pulso completo foi recebido dentro do tempo limite.

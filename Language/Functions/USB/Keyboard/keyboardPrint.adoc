...
[float]
=== Parâmetros
`character` : um char ou int a ser enviado ao computador como se fosse uma tecla pressionada no teclado. + 
`characters` : uma string a ser enviada ao computador como se fosse uma palavra ou frase digitada no teclado.

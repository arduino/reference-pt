...
[float]
=== Descrição
Configura a taxa de transferência em bits por segundo (baud rate) para transmissão serial. Para comunicação com um computador, através do monitor serial, use uma das velocidades de transmissão listadas no canto inferior direito da janela do mesmo. Você pode, no entanto, especificar outras taxas para comunicar nos pinos 0 e 1. Isso permite comunicar-se com componentes que requerem uma taxa de transferência peculiar, por exemplo.

...
[float]
== Usando Interrupções
Interrupções são úteis para fazer coisas automaticamente em programas de microcontroladores, e podem ajudar a resolver problemas de temporização. Boas tarefas para se usar uma interrupção podem icluir a leitura de um codificador rotativo, ou monitorar entradas de dados pelo usuário.
